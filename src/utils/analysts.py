...
from typing import Dict, Any, Tuple  # Import at the top
...
}  # Ensure dictionary is closed correctly


# Helper function for sorting analysts by order
def get_analyst_sort_key(item: Tuple[str, Dict[str, Any]]) -> int:
    """Returns the 'order' value from the analyst config, defaulting to 0."""
    key, config = item
    ...
    if isinstance(order, str) and order.isdigit():
        return int(order)
    # Default case if 'order' is missing or not an int/int-string
    return 0


# Derive ANALYST_ORDER from ANALYST_CONFIG using the helper function
ANALYST_ORDER = [
    (config["display_name"], key)
    for key, config in sorted(ANALYST_CONFIG.items(), key=get_analyst_sort_key)
]
