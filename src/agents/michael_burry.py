...
def michael_burry_agent(state: AgentState) -> dict:  # noqa: C901  (complexity is fine here)
...
def _latest_line_item(line_items: list) -> Any | None:
...
def _analyze_value(metrics, line_items, market_cap) -> dict:
...
def _analyze_balance_sheet(metrics, line_items) -> dict:
...
def _analyze_insider_activity(insider_trades) -> dict:
...
def _analyze_contrarian_sentiment(news) -> dict:
